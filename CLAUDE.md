...
- `tools/build.mjs` — templates (layout, nav, footer), the page builders, SEO `<head>`, JSON-LD, sitemap/robots. A tiny inline Markdown converter (`md()`) handles article bodies.
  - **Every page is a "drawing sheet" takeover.** Each builder passes `layout({ bare: true, bodyClass: 'jz', … })`, so pages carry **no** `.ks-nav`/`.ks-footer`; instead `jzChrome(sheetTitle, dwgNo, current, depth)` renders the fixed drawing-sheet frame (blueprint grid, ruled border, header rule with the site nav folded in, bottom title block), and content lives in an inner `.jz-scroll` region. The two site fonts load globally from `layout()`'s `<head>` (Saira Condensed + IBM Plex Mono). All styling is the `.jz-*` system at the end of `site.css` (self-scoped palette + fonts under `body.jz`); shared building blocks include `.jz-hero`/`.jz-h1`/`.jz-h2`/`.jz-lede`/`.jz-body-copy`/`.jz-sec`/`.jz-rows`/`.jz-works`/`.jz-photoplate`/`.jz-metacells`/`.jz-plate`/`.jz-form`/`.jz-dl`/`.jz-btn`/`.jz-marquee`/`.jz-foot`. Sheet numbering by section: home `GA-00`, projects `P-00`/`P-01…`, studio `S-01`, contact `C-01`, apply `R-01`, journal `A-00`/`A-01…`, 404 `X-404`.
  - The sheet is viewport-locked (`body.jz { height:100dvh; overflow:hidden }`) and only the inner `.jz-scroll` region scrolls — do **not** switch this to document-level scroll under the fixed frame (it triggers a compositor repaint/blank-frame bug).
  - The old page-specific CSS (`.home-c__*`, `.subp*`, `.projects__grid`, `.about__*`, `.contact__*`, `.apply__*`, `.article*`, `.journal-*`, `.ks-nav`, `.ks-footer`, …) is now **dead** (no builder emits it) but left in `site.css` for now; it can be pruned. The projects filter still uses the hook classes `.projects__filter`/`.projects__card`/`.projects__count` (kept on the new `.jz-*` markup) so `assets/js/site.js` keeps working. Journal margin-note copy per post is in `journalNotes` (data.mjs). Zine interactions (ball hop, random-sheet jump, "plotting" transition on plate click) live in the `body.jz` block of `assets/js/site.js`.
...
- Fonts load from Google Fonts — just two: **Saira Condensed** + **IBM Plex Mono** (loaded once in `layout()`'s `<head>`).
